...
class SquareHitMark(Squad, HitMark):
...
class GameLogic:
    ...
    def processEvent(self, event):
        if event.type == Proces.Event_Tick:
        ...
        if event.type == Proces.Event_Hit:
            self.hit(event.data)
...
class PyGameGui:
    ...
    def run(self):
        running = True
        pygame.time.set_timer(pygame.USEREVENT + Proces.Event_Tick, 1000)
        while running:
    ...
    def processEvent(self, event):
        if (event.type >= pygame.USEREVENT) and (event.type < pygame.NUMEVENTS):
            myevent = Proces(event.type - pygame.USEREVENT, None)
            self.logic.processEvent(myevent)

        if event.type == pygame.MOUSEBUTTONDOWN:
            pypos = event.pos
            myevent = Proces(Proces.Event_Hit, Pos(pypos[0], pypos[1]))
            self.logic.processEvent(myevent)
